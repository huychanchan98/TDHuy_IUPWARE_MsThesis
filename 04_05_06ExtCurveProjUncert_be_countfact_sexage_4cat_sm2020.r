...
# TOTAL DEATHS FOR EACH CATEGORY
death_0_64<-obs$all_0_64
death_65_74<-obs$all_65_74
death_75_84<-obs$all_75_84
death_85plus<-obs$all_85plus

death_cat<-cbind(death_0_64,death_65_74,death_75_84,death_85plus)
...
for (i in seq(m_category)){
  ...
  scen_fact <- c(0,2.06,2.65,1.09,2.17) #2.65 (HADCrut5), (-0.1 compare to Swiss)
    ...
    # LOOP ACROSS ITERATIONS
      ...
      # COMPUTE THE DAILY CONTRIBUTIONS OF ATTRIBUTABLE DEATHS
        an <- (1-exp(-bvarcen%*%coefsim[s,]))*datasel
...
# COMPUTE AN/AF (95%CI) IN THE ENSEMBLE, BY RANGE & PERIOD

# CREATE NEW OBJECTS TO STORE RESULTS
# We now create 4 new arrays (2 arrays to store the abs AN, AF, factual and counterfactual mean ) 
# with 4 dimensions each to store the 
#   ensemble estimates (average impacts across GCMs) with the empirical 
#   95% confidence intervals. 
# In this case, the 4 dimensions correspond to the research period, the point estimate 
...
barplot <- ggplot(combined_data, aes(x=age_group, y=est, fill=dataset)) + 
  ...
  labs(x = "Age Group", y = "Heat-related Mortality (%)",
       #title = "Heat-related Mortality by Age Group (Summer 2020)") +
        title = "Heat-related Mortality by Male Group (Summer 2020)") +
       # title = "Heat-related Mortality by Female Group (Summer 2020)") +
  theme_minimal() +
...
print(barplot)
# Save the plot
#ggsave("Heatmort_bel_age_sm2020.pdf", plot = barplot, width=10, height=6)
#ggsave("Heatmort_bel_male_age_sm2020.pdf", plot = barplot, width=10, height=6)
#ggsave("Heatmort_bel_female_age_sm2020.pdf", plot = barplot, width=10, height=6)

# NB: uncomment ggsave for saving the plot in pdf format.
